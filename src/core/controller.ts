...
export class Controller {
  ...
  private async load(id: string, reconnect = false) {
    if (this.disposed) return
  ...
  private async readDirty() {
    ...
    try {
      ...
      for (const id of ids) {
        ...
        let info: Info
        try { info = await this.source.get(id, controller.signal) }
        catch (error) {
          if (generation !== this.generation || this.disposed || controller.signal.aborted) return
      ...
      if (!controller.signal.aborted && generation === this.generation) { controller.abort(); this.state = "stale" }
    } finally {
      if (generation === this.generation && !this.disposed) {
        this.request = undefined
        this.journal = undefined
