...
import type { Event, Info, Snapshot, Source } from "./types.js"
import { MissingSession } from "./types.js"

type Incoming = Parameters<Source["listen"]>[0] extends (event: infer E) => void ? E : never
export type LoadState = "loading" | "ready" | "error" | "stale"
...
export class Controller {
  ...
  constructor(private source: Source, private changed: () => void, private clock = Date.now, private retained?: Map<string, Tree>) {
    this.trees = retained ?? new Map()
    // 必须先订阅；祖先解析和首个 HTTP 快照期间也要保留事件。
    this.stop = source.listen(event => this.receive(event))
  }

  private knows(tree: Tree, event: Event) {
    return tree.nodes.has(event.sessionID) || tree.deleted(event.sessionID) || (event.kind === "deleted" && tree.hasPending(event.sessionID)) ||
      (event.kind === "created" && !!event.info.parentID && tree.nodes.has(event.info.parentID))
  }

  private receive(event: Incoming) {
    ...
    for (const tree of this.trees.values()) {
      ...
      if (tree === this.current) {
        ...
        if (tree.invalidAll) { this.fullQueued = true; this.schedule() }
  ...
  private discover(tree: Tree, revisions: Map<string, number>, info: Info) {
    if (!revisions.has(info.id)) revisions.set(info.id, tree.nodes.get(info.id)?.revision ?? 0)
    tree.discover(info)
    this.journal?.drain(tree)
  }

  private commit(tree: Tree, revisions: Map<string, number>, snapshots: Snapshot[]) {
    let complete = true
  ...
  private status(tree: Tree): LoadState {
  ...
  private async load(id: string) {
    ...
    const revisions = new Map([...this.trees.values()].flatMap(tree => [...tree.nodes].map(([key, node]) => [key, node.revision] as const)))
    this.changed()
    try {
      ...
      const snapshots = await readTree(this.source, ancestor, controller.signal,
        info => { if (generation === this.generation && !controller.signal.aborted) this.discover(tree, revisions, info) },
        id => { if (generation === this.generation && !controller.signal.aborted) tree.missing(id) },
      ...
      if (complete) { tree.invalidAll = false; tree.recover(tree.rootID, new Set(snapshots.map(snapshot => snapshot.info.id))) }
  ...
  private schedule(id?: string) {
    ...
    this.timer = setTimeout(() => { this.timer = undefined; void this.readDirty() }, 150)
  }

  private async readDirty() {
    ...
    try {
      ...
      for (const id of ids) {
        ...
        try {
          ...
          const snapshots = await readTree(this.source, info, controller.signal,
            info => { if (generation === this.generation && !controller.signal.aborted) this.discover(tree, revisions, info) },
            id => { if (generation === this.generation && !controller.signal.aborted) tree.missing(id) },
          ...
          if (generation !== this.generation || this.disposed || controller.signal.aborted) return
          if (error instanceof MissingSession && error.sessionID === id && id !== tree.rootID && id !== this.selected) tree.missing(id)
          else if (error instanceof FullReadError || id === tree.rootID || (error instanceof MissingSession && id === this.selected)) {
