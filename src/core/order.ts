...
export class EventOrder {
  ...
  private lifecycleTime = -Infinity
  private active = new Map<string, number>()
  private settled = new Map<string, number>()
  private retiredThrough = -Infinity
  private calls = new Map<string, number>()
  private refresh = false

  private rememberSettled(key: string, at: number) {
  ...
  reconcileRequests(permissions: Iterable<string>, forms: Iterable<string>) {
  ...
  takeRefresh() { const refresh = this.refresh; this.refresh = false; return refresh }
  get retainedIDs() { return this.recent.size }
  get settledRequests() { return this.settled.size }
  get retainedCalls() { return this.calls.size }
}
