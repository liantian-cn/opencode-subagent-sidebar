import type { Assistant, Event, Info, Link, Outcome, Row, Snapshot } from "./types.js"
import { EventOrder } from "./order.js"

interface Round {
  phase: "running" | "unknown" | "ended"
  started?: number
  ended?: number
  outcome?: Outcome
  history: boolean
...
export class Tree {
  ...
  deleted(id: string) { return this.tombstones.has(id) }
  hasPending(id: string) { return this.pendingLinks.has(id) }
  pendingIDs() { return [...this.pendingLinks.keys()] }
  requireSync(id: string) {
  ...
  snapshot(snapshot: Snapshot, revision: number) {
  ...
  event(event: Event) {
    ...
    switch (event.kind) {
      ...
      case "step":
        ...
        break
      case "retry": round.retryAt = event.until; break
      case "retry-clear": round.retryAt = undefined; break
      case "permission":
      case "form": {
        const pending = event.kind === "permission" ? node.permissions : node.forms
        if (event.pending) pending.add(event.requestID)
        else pending.delete(event.requestID)
        break
      }
      case "info": Object.assign(node.info, event.info); break
      case "link": this.associate(event.link); break
    }
  }
  ...
  rows(_now?: number): Row[] {
    ...
    return rows.map(node => {
      ...
      return {
        ...
        status, started: round.started, ended: round.phase === "ended" ? round.ended : undefined,
      }
    })
  }
}
